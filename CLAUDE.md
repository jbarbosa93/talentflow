...
## Version actuelle
**v2.9.38 (TalentFlow Mobile — PWA consultant Phase 1)** — 21/05/2026

### v2.9.38 — TalentFlow Mobile (PWA consultant) Phase 1
- Dashboard installable en app « TalentFlow » : `manifest.json` renommé (name `TalentFlow`, `start_url:/dashboard`), barre de navigation basse mobile (`MobileBottomNav` — 6 sections Accueil/Candidats/Clients/Missions/Signatures/Rapports), bandeau d'installation (`MobileInstallPrompt`, `/dashboard` only). Visible ≤768px (CSS `dashboard.css`). Phase 2 = polish mobile par page (pattern #85).
...
- **v2.9.30** : fix signature consultant — `PublicFieldsLayer` recevait l'ordre brut au lieu de `effectiveRecipientOrder` → le consultant voyait le champ signature DU candidat (pattern #71).
- **v2.9.31** : email unique de finalisation (docs signés + pièces jointes en 1 mail au créateur), fix `.txt` (extension `.pdf` manquante), dédup pièces jointes (valeur recopiée sur plusieurs tokens), orientation EXIF des photos iPhone, nommage des fichiers d'après le champ (pattern #83).
...
**79. SIGN — Gestion des sections sans entité DB** (v2.9.21) — Une « section » n'est PAS une entité : c'est la chaîne `wizardSection` posée sur chaque champ. Le panneau « Gérer les sections » (`components/sign/SectionManager.tsx`, modal partagé Mode Wizard + Mode Document) repose entièrement là-dessus, **zéro migration**. Helpers dans `lib/sign/section-helpers.ts` : `collectSections()` (sections dans l'ordre d'apparition), `moveSectionBlock()` (réordonne en déplaçant le BLOC contigu d'une section dans `step.fieldIds` — un bloc = champs consécutifs de même section ; échange avec le bloc voisin ; retourne la MÊME référence si déplacement impossible → sert aussi à calculer `canMoveUp/Down`), `loadCollapsedSections/saveCollapsedSections` (état replié en `localStorage` clé `sign:collapsed-sections:{templateId}`, partagé entre les 2 modes). Règles : (a) renommer/supprimer itère sur **tous les documents** (une section peut s'étendre sur plusieurs PDFs) — Mode Document utilise des patchers cross-doc `patchFieldsAcrossDocs/deleteFieldsAcrossDocs` qui poussent l'history + dirty ; (b) le réordonnancement n'existe **qu'en Mode Wizard** (le Mode Document est positionné en absolu sur le PDF) ; (c) supprimer = 2 choix : « Dégrouper » (vide `wizardSection`, garde les champs) ou « Supprimer les champs » (destructif + `confirm()`) ; (d) « replier » est une convenance d'édition (masque les champs dans l'éditeur / cache le badge Konva via prop `collapsedSections` de `FieldsCanvas`) — **jamais** envoyé au candidat ; (e) « Tout obligatoire » exclut les checkboxes groupées (pattern #78).

**80. SIGN — Champ `attachment` fonctionnel + intégration Conformité** (v2.9.23) — Le type `attachment` (pièce jointe) était défini mais mort (rendu comme un input texte, jamais collecté). Rendu fonctionnel **sans migration DB** :
- **Widget** `components/sign/AttachmentField.tsx` (partagé Wizard inline / Document via modal portalisé). Valeur stockée dans `sign_tokens.field_values[fieldId]` au format `SignAttachmentValue = { files: SignAttachmentFile[] }` (path Storage + name + size + readable + expiryDate).
- **Upload** : route PUBLIQUE `/api/sign/attachment-url` (token-in-body, comme `verify-token`/`finalize`) → URL signée Supabase → le navigateur PUT le fichier direct dans `talentflow-sign/uploads/{envelopeId}/{tokenId}/` (pas de limite Vercel 4,5 Mo).
...
**82. SIGN — Composition recto/verso en PDF** (v2.9.25, étendu v2.9.31) — `lib/sign/compose-attachment-pdf.ts` `composeImagesToPdf()` : assemble des images JPEG/PNG en un PDF A4 via pdf-lib — **1 image → pleine page**, **2 images → empilées** (recto haut / verso bas). Utilisé par `processCandidateUploads` : les images d'un MÊME champ pièce jointe → 1 PDF « type scan ». `embedJpg`/`embedPng` uniquement (webp/heic/pdf laissés bruts, try/catch fallback). La Conformité garde recto/verso comme 2 faces distinctes (modèle `file_recto_path`/`file_verso_path`). **v2.9.31 — orientation EXIF corrigée** : `readJpegOrientation()` (parser EXIF autonome, sans dépendance — scanne le marqueur APP1, tag `0x0112`) → la rotation (0/90/180/270) est appliquée via `drawImage({ rotate: degrees() })` avec ajustement du pivot bas-gauche. Orientations 6/8 inversent largeur/hauteur d'affichage.

**83. SIGN — Email unique de finalisation + dédup pièces jointes** (v2.9.31) — À la complétion d'une enveloppe, le **créateur reçoit UN SEUL email** (`sendSignFinalRecapEmail`, design logo L-Agence) regroupant : documents signés + pièces jointes chargées par le candidat. Avant : 2 emails séparés. Mécanique dans `app/api/sign/finalize/route.ts` : (a) `stampAllAndSendEmails` exclut le `creatorEmail` de l'envoi `sendSignCompletedEmail` standard et retourne `{ paths, signedAttachments }` ; (b) `processCandidateUploads` n'envoie plus d'email, il **retourne** `{ attachments, uploaderName }` ; (c) le bloc finalize envoie l'email fusionné à `creatorEmail = senderInfo.email || ADMIN_EMAIL`. Le candidat (et les CC) ne reçoivent QUE leurs documents signés — jamais les scans du candidat. 3 pièges corrigés : (1) **extension `.pdf`** — `d.name` (nom de doc) n'a pas toujours `.pdf` → sans elle, le client mail traite la PJ comme `.txt` → helper `ensurePdfFilename()` ; (2) **dédup** — la valeur d'un champ `attachment` est recopiée sur PLUSIEURS tokens (le candidat la remplit, le consultant la voit en « valeur précédente » et son token la réenregistre) → collecter par champ en **dédup sur `file.path`**, jamais une boucle `field × tokens` naïve ; (3) **uploaderName** — trier les tokens NON-créateur d'abord pour que le nom affiché soit le vrai candidat, pas le créateur. Nommage des fichiers basé sur le champ (`field.tooltip || field.label` → « CV.pdf », « Permis de travail.pdf »), pas le nom brut du fichier uploadé.

**85. PWA — TalentFlow Mobile (app consultant)** (v2.9.38) — Le dashboard (`/(dashboard)`, scope `/`) est installable en app « TalentFlow ». Réutilise le `public/manifest.json` existant (renommé `name:"TalentFlow"`, `start_url:"/dashboard"`, icônes `icon-192/512.png` = éclair TalentFlow). Pièces : (a) `components/layout/MobileBottomNav.tsx` — barre de navigation basse 6 sections (Accueil `/dashboard` · Candidats · Clients · Missions · Signatures `/sign` · Rapports `/sign/rapports`) ; désambiguïsation `/sign` vs `/sign/rapports` dans `isActive()` ; (b) `components/layout/MobileInstallPrompt.tsx` — bandeau d'install (Android `beforeinstallprompt` / tuto iOS), visible `/dashboard` only, dismiss 14j, positionné au-dessus de la barre de nav ; (c) CSS `.d-bottom-nav` dans `dashboard.css` — `display:none` desktop, `flex` ≤768px (breakpoint mobile du dashboard) + `.d-content` `padding-bottom` pour ne pas masquer le contenu. Les 2 composants montés dans `DashboardShell`. **Pas de service worker** (Chrome moderne installe via manifeste seul ; ne PAS réveiller le kill-switch `sw.js`). Phase 1 = installable + navigable ; Phase 2 (à venir) = optimisation mobile page par page. Distinct du pattern #84 (PWA Rapport candidat, scope `/report`).

**84. PWA — Portail rapport candidat installable (« TalentFlow Rapport »)** (v2.9.35) — Le portail rapport candidat (`/report/*`, qui a sa propre racine `app/report/layout.tsx` rendant `<html>`) est installable en app. Architecture **additive** — le site web reste 100% fonctionnel dans tous les navigateurs, l'install est un bonus. Pièces : (a) `public/report.webmanifest` (manifeste DÉDIÉ, `scope`/`start_url` = `/report`, distinct de `public/manifest.json` qui est l'app ATS dashboard) ; (b) `public/sw-report.js` — service worker **scope `/report`, ZÉRO cache** (handler `fetch` vide). ⚠️ Un ancien SW global (`public/sw.js`, désormais kill-switch) cachait des réponses porteuses de cookies → incident 494 `REQUEST_HEADER_TOO_LARGE`. **NE JAMAIS** cacher de réponse cookie/auth dans un SW. (c) `components/report/ServiceWorkerRegister.tsx` enregistre le SW (scope `/report` — narrowing autorisé sans header) + mémorise le slug courant en `localStorage` clé `tf_report_last` ; (d) `components/report/PwaInstallPrompt.tsx` — bandeau bas : Android capture `beforeinstallprompt` → bouton « Installer » ; iOS (pas d'install auto possible) → tuto illustré Partager→Sur l'écran d'accueil ; masqué si `display-mode: standalone` ou rejeté <14j ; visible seulement sur `/report/{slug}` ; (e) `app/report/page.tsx` — page d'entrée : redirige vers `/report/{tf_report_last}` (l'app Android s'ouvre sur `start_url` `/report`) ou message « ouvre ton lien ». Métadonnées dans `app/report/layout.tsx` : `manifest` + `appleWebApp` (génère les metas Apple — ne pas dupliquer en `<head>` manuel). Icônes réutilisées : `/icon-192.png` + `/icon-512.png`.
