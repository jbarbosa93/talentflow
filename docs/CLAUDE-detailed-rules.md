...
- **Cas résiduels v1.9.105 résolus en v1.9.107** (banc 22 : 19→22, témoin 100 : 58→60) :
  - **DOCX** : `extractPhotoFromDOCX` instrumenté logs `[F5-DOCX]` + extension F1bis-DOCX (oversizedScans → tryVisionFaceCrop sur grandes photos `word/media/*`, max 2 photos pour coût). Cas Soraia : photos téléphone 4032×3024 (paysage, ratio 0.75) skipped silently, désormais Vision face crop.
  - **FlateDecode** : `processXObjects` étend F1bis pour FlateDecode portrait pleine page (ratio 1.3-1.55, ≥1500×2000). Décompression zlib + résolution colorSpace/bpc/channels (mêmes règles que bloc FlateDecode standard) + ré-encodage JPEG via sharp pour pouvoir le passer à `tryVisionFaceCrop`. Cas Amélie Gorin : scan A4 PNG/raw stocké en FlateDecode pleine page.
  - **Veto `uniqueColors<40`** : assoupli pour `source.startsWith('vision-face')` à seuil `uc≥35`. Vision a déjà confirmé visuellement le visage avant le crop — le veto motif_decoratif est trop strict pour les scans basse qualité (cas José Antonio Ruiz uc=39). Sub-35 maintient le veto (filet anti-faux-positif).
  - **Normalisation source** : `tryVisionFaceCrop` préfixe désormais `vision-face:` devant le sourceLabel (sauf si déjà préfixé). Permet à `scoreHeadshot` + `explainScore` d'identifier les crops Vision (`startsWith('vision-face')`). Strategy 3 inline (PDF, source `vision-face` hardcodée) est compatible.
  - **Fix bonus face cover ratio** : ancien check `cropW < origW*0.4 && cropH < origH*0.4` calibré pour scans portrait, faux-rejette les photos paysage (cropH ratio 1.25 explose vs origH < origW). Nouveau : `faceSize / Math.max(origW, origH) > 0.5 → reject` (probable photo passport déjà cropée). Orientation-agnostique. Filet final : `scoreHeadshot` (uniqueColors, skinRatio, ratio).
