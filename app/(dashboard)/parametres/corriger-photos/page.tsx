...
import { Play, Square, CheckCircle, XCircle, Camera, Loader2, RefreshCw, ThumbsUp, ThumbsDown, User } from 'lucide-react'
    ...
    <div className="d-page" style={{ maxWidth: 860, paddingBottom: 60 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          ...
          <p className="d-page-sub" style={{ margin: 0 }}>Analyse les CVs et valide chaque photo trouvée en temps réel</p>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          {phase === 'running' ? (
            ...
            <button onClick={handleStart} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 10, fontSize: 13, fontWeight: 700, border: 'none', background: 'linear-gradient(135deg, var(--primary), #E8940A)', color: '#0F172A', cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 2px 8px rgba(245,167,35,0.3)' }}>
              {phase === 'done' ? <><RefreshCw size={14} /> Relancer</> : <><Play size={14} fill="#0F172A" /> Lancer l&apos;analyse</>}
            </button>
          )}
          {phase === 'running' && (
      ...
      {/* Empty state while running, no photos found yet */}
      ...
      {phase !== 'idle' && phase !== 'running' && pendingCount === 0 && (approved + rejected) > 0 && (
        <div style={{ marginTop: 20, padding: '20px 24px', borderRadius: 12, background: '#F0FDF4', border: '1.5px solid #BBF7D0' }}>
