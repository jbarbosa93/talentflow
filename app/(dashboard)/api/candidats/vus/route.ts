...
export async function DELETE(request: NextRequest) {
  ...
  if (authError) return authError
  try {
    // v1.9.95 — flag `all_users` ignoré (rétrocompat) : DELETE est désormais strictement per-user.
    // Règle absolue : "vu/non-vu" est per-user, ne modifie jamais l'état des autres consultants.
    const { ids } = await request.json() as { ids: string[]; all_users?: boolean }
    if (!Array.isArray(ids) || ids.length === 0) return NextResponse.json({ ok: true })
