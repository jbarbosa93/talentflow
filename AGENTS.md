# TalentFlow — AGENTS.md

> **Détails techniques** : `docs/Codex-detailed-rules.md` (patterns complets + routes API)
> **Historique complet** : `docs/Codex-history.md` (changelog v2.6.x → v2.9.45, audits sécurité, dette technique)
> **Sessions/versions** : `~/.Codex/.../memory/MEMORY.md` (index condensé) + `memory/session_*.md` (détails)
...
|---|---|---|
| Bug fix ciblé, correction CSS, typo, rename | Codex-haiku-4-5 | Rapide, pas besoin de raisonnement profond |
| Nouveau composant UI, route API simple, refacto isolé | Codex-sonnet-4-5/6 | Bon équilibre vitesse/qualité |
| Architecture nouvelle, migration DB complexe, refacto multi-fichiers, logique métier critique | Codex-sonnet-4-5/6 | Raisonnement étendu activé |
| Audit complet, plan technique multi-phases, décisions irréversibles (migrations prod, breaking changes) | Codex-opus-4-5/7 | Analyse maximale |
...
### Mise à jour MEMORY.md et AGENTS.md
À chaque fin de session (avant déploiement) :
1. Mettre à jour `MEMORY.md` (1-ligne par session)
2. Mettre à jour `AGENTS.md` (version + changelog condensé si pattern nouveau)
...
→ **Historique exhaustif** v2.6.x → v2.9.45 : `docs/Codex-history.md`
...
- **genre** → **IMMUABLE** (Codex se trompe souvent)
...
- **IA** : Codex API (Anthropic), Google Generative AI, Groq — parsing CV et matching
...
docs/                 — docs étendues (Codex-detailed-rules.md, Codex-history.md)
```
...
⚠️ Middleware exclut toutes les routes `/api/`. Protection via `requireAuth()` dans chaque route. **Liste exhaustive + détails par catégorie** : `docs/Codex-detailed-rules.md`.
...
**Liste exhaustive (85+ patterns)** dans `docs/Codex-detailed-rules.md`. Top patterns à connaître :
...
→ **Détails complets** : `docs/Codex-history.md`
